...
def main():
    ...
    gt_action = gt_raw.reshape(-1, 5, 2).mean(1)[0]  # (2,)
    gt_actions_t = (torch.from_numpy(gt_action).unsqueeze(0).unsqueeze(0)
    ...
    axes[1, 0].set_title(f"Goal (frame 3)\nPlanner: {best_action.round(3)}\nGT: {gt_action.round(3)}")
    axes[1, 0].axis("off")
